...
class DT:    
    ...
    def NavigateGuild(self, guild_obj:DTGuild):
        time.sleep(1)
        self.NavigateHome()
        global sl_element_list
        global server_list
        global sl
        self.GetGuilds()
        nm = str(guild_obj.name)
        if nm in sl_element_list:
            logging.info(f"[DT-SUCCESS]{guild_obj.name} is a Valid Guild")
            server_list[sl_element_list.index(guild_obj.name)].click()
        else:
            raise GuildNotFound(f"'{guild_obj.name}' is not a Valid Guild")
    ...
    def NavigateChannel(self,chnl_obj:DTChannel):
        if(chnl_obj.index != None):
            self.__NavigateChannelByIndex(chnl_obj)
        else:
    ...
    def __NavigateChannelByIndex(self,chnl_obj:DTChannel):
        guild = chnl_obj.guild
        ind = chnl_obj.index
        global channel_list
        global channel_element_list
        ...
        try:
            ...
            raise ChannelNotFound(f"[DT-ERROR] {chnl_obj.channel} Is Not A Valid Channel")
    ...
    def GetMemberCount(self , guild:DTGuild):
        self.__NavigateChannelByIndex(DTChannel(guild,index=0))
        soup = BeautifulSoup(self.driver.page_source , "html.parser")
    ...
    def GetSortedMembers(self,guild:DTGuild):
        ...
        rd = dict(self.GetMemberCount(guild)[1])
